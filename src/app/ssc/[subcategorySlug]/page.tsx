...
export default function SSCSubcategoryPage() {
  const params = useParams<{ subcategorySlug: string }>();
  const router = useRouter();
  const subcategorySlug = params?.subcategorySlug?.toLowerCase();
  ...
  useEffect(() => {
    const fetchSubcategory = async () => {
      if (!subcategorySlug) {
        setError("Missing subcategory slug");
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const endpoints = [
          buildApiUrl(`/taxonomy/category/${CATEGORY_SLUG}/subcategory/${subcategorySlug}`),
          buildApiUrl(`/taxonomy/subcategory-id/${subcategorySlug}`)
        ];

        let resolved: any = null;
        for (const endpoint of endpoints) {
          if (process.env.NODE_ENV !== "production") {
            console.log("[SSC] Resolving subcategory", { endpoint, subcategorySlug });
          }
        ...
        setError(err.message || "Unable to load subcategory");
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchPageContent = async () => {
      ...
      try {
        ...
        if (process.env.NODE_ENV !== "production") {
          console.log("[SSC] Fetching page content", { endpoint, subcategoryId });
        }
        const res = await fetch(endpoint);
        if (!res.ok) {
  ...
  const handleDownloadExamPDF = async (examId: string) => {
    try {
      ...
      console.error('[SSC] exam PDF generation failed', err);
  ...
  useEffect(() => {
    const fetchMockTests = async () => {
      ...
      try {
        ...
        const endpoint = buildApiUrl(`/taxonomy/category/${CATEGORY_SLUG}/subcategory/${subcategorySlug}/exams?${params.toString()}`);
        ...
        console.error('[SSC] mock tests fetch failed', err);
        setMockTests([]);
      } finally {
  ...
  useEffect(() => {
    const fetchQuestionPapers = async () => {
      ...
      try {
        ...
        console.error('[SSC] question papers fetch failed', err);
        setQuestionPapers([]);
      } finally {
  ...
  const heroTitle = useMemo(() => subcategoryInfo?.name || subcategorySlug?.toUpperCase(), [subcategoryInfo, subcategorySlug]);
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center">
        <p className="text-2xl font-semibold text-gray-800">Unable to load this SSC section.</p>
    ...
    <div className="space-y-4">
      {["Notification Released", "Application Window Opens", "Exam Pattern Updated"].map((item, index) => (
        <div key={item} className="flex items-start">
          ...
          <div className="ml-3">
            <p className="text-sm text-gray-700">SSC CHSL {item}</p>
...
const DefaultSSCPage = ({ title, subtitle }: { title: string; subtitle: string }) => (
  <div className="min-h-screen bg-gray-50">
    <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center">
