...
interface Props {
  slug: string;
  initialArticle: Blog | null;
  initialSections: BlogSection[];
}

export default function CurrentAffairsDetailClient({ slug, initialArticle, initialSections }: Props) {
  const router = useRouter();
  const [article, setArticle] = useState<Blog | null>(initialArticle);
  ...
  useEffect(() => {
    ...
    loadNote();
  }, [slug, router, initialArticle]);

  if (isLoading) return <LoadingPage message="Loading note..." />;
