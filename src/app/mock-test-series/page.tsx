...
export default function MockTestSeriesPage() {
  ...
  useEffect(() => {
    fetchSeries();
  }, [filters.category, filters.subcategory, pagination.page, debouncedSearch]);

  useEffect(() => {
    const handle = setTimeout(() => setDebouncedSearch(filters.search.trim()), 400);
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      setCategories(
        data.filter((c) => c.is_active !== false).sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0))
      );
    } catch (err) {
      ...
      <section className="gradient-hero py-10">
        <div className="w-full px-4 sm:px-8 lg:px-12 xl:px-20 2xl:px-32">
          <div className="max-w-3xl">
            <Breadcrumbs items={[HomeBreadcrumb(), { label: 'Mock Test Series' }]} variant="dark" className="mb-6" />
            <h1 className="font-display text-4xl md:text-5xl font-bold text-background mb-4">
              Mock Test Series
            </h1>
            <p className="text-lg text-background/80 mb-8">
              Full-length mock test series mapped to the latest exam patterns. Practice, analyse, and improve.
            </p>
            <form onSubmit={(e) => { e.preventDefault(); setPagination((p) => ({ ...p, page: 1 })); }}
              ...
              <div className="relative flex-1">
                ...
                <Input type="text" placeholder="Search test series by name or category..."
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
              <div>
                <h2 className="font-display text-2xl font-bold text-foreground">Test Series</h2>
                <p className="text-sm text-muted-foreground">
                  Showing {testSeries.length} of {pagination.total} test series
                </p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2 text-muted-foreground text-sm">
                  ...
                  <span>{pagination.total} series</span>
                </div>
              </div>
            </div>
            ...
            {error && (
              <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-4 mb-6">
                <p className="text-destructive font-medium">Error loading test series</p>
                <p className="text-destructive/80 text-sm mt-1">{error}</p>
                <Button variant="outline" size="sm" onClick={() => { setError(''); fetchSeries(); }} className="mt-2">
                  Try Again
                </Button>
              </div>
            )}
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12 bg-card rounded-2xl border border-border">
                ...
                <h3 className="font-display text-2xl font-bold mb-2">No test series found</h3>
                <p className="text-muted-foreground mb-6">
                  {hasCustomFilters
                    ? 'No test series match your current filters. Try adjusting your search criteria.'
                    : 'Test series are being added. Please check back later.'}
                </p>
