...
/** Stable anchor id from any string */
const toAnchorId = (s: string) =>
  'toc-' + s.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');

/** Extract first h2/h3 text from an HTML string */
...
export default function BlogDetailPage() {
  ...
  const fetchArticleDetails = async () => {
    ...
    try {
      ...
      if (data.is_current_affairs_note) { window.location.replace(`/current-affairs/${data.slug}`); return; }
      setArticle(data);
      const content = await blogService.getBlogContent(data.id);
      setSections(content);
      if (data.category) {
        const related = await blogService.getBlogs({ category: data.category, limit: 7 });
        setRelatedArticles(related.data.filter(b => b.id !== data.id).slice(0, 3));
      }
    } catch (err: any) {
  ...
  const fetchLatestBlogs = async () => {
    try { const r = await blogService.getBlogs({ limit: 6 }); setLatestBlogs(r.data); } catch {}
  };

  const fetchCategories = async () => {
    try { setCategories((await blogService.getCategories()).slice(0, 8)); } catch {}
  };
  ...
  /**
   * Build TOC entries — one per block that has a meaningful heading.
   * Each entry gets a stable anchor id like "toc-0-1" (sectionIdx-blockIdx).
   * We place a <span id="..."> before the block in the DOM — no HTML patching needed.
   */
  const tocItems = useMemo(() => {
    const items: { id: string; label: string; level: number }[] = [];
    sections.forEach((section, si) => {
      // Section title itself becomes a TOC entry
      if (section.title) {
  ...
  if (isLoading) return <LoadingPage message="Loading blog..." />;

  if (error || !article) {
    return (
      <div className="min-h-screen bg-background py-12">
        <div className="container-home text-center">
          <h2 className="font-display text-2xl font-bold text-foreground mb-4">{error || 'Blog not found'}</h2>
          <Link href="/blogs"><Button><ArrowLeft className="h-4 w-4 mr-2" />Back to Blogs</Button></Link>
        </div>
      </div>
    );
  }

  // Shared TOC nav items renderer
  const TocNav = () => (
  ...
  const shareUrl = typeof window !== 'undefined' ? window.location.href : `https://bharatmock.com/blogs/${slug}`;
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-4 md:py-8">
        <div className="container-home">
          <Breadcrumbs
            items={[HomeBreadcrumb(), { label: 'Blogs', href: '/blogs' }, { label: article.category || 'Article' }]}
            variant="dark"
            className="mb-2 md:mb-3"
          />
          {article.category && (
            <span className="inline-block bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-sm mb-2 md:mb-3">
              {article.category}
            </span>
          )}
          ...
          <div className="flex flex-wrap items-center gap-3 md:gap-4 text-xs md:text-sm text-blue-100">
            ...
            {article.published_at && (
              <span className="flex items-center gap-1"><Calendar className="h-3 w-3 md:h-3.5 md:w-3.5" />{formatDate(article.published_at)}</span>
            )}
            {article.read_time && (
              <span className="flex items-center gap-1"><Clock className="h-3 w-3 md:h-3.5 md:w-3.5" />{article.read_time} min read</span>
            )}
          ...
          <main className="space-y-4 min-w-0">
            ...
            {sections.length > 0 ? (
              sections.map((section, si) => (
                <div key={section.id}>
                  {/* Anchor for section title TOC entry */}
                  {section.title && <span id={toAnchorId(`section-${si}`)} className="block" />}
                  <div
                    className="bg-card border border-border rounded-lg p-4 md:p-5 rich-text-content"
                    ...
                    <div className="space-y-3">
                      {section.blocks.map((block, bi) => {
                        ...
                        const hasAnchor =
                          ...
                          ((block.block_type === 'rich_text' || block.block_type === 'html') &&
                            !!firstHeadingText(block.content?.html || ''));
                        return (
                          <div key={block.id}>
                            {/* Invisible anchor span placed before the block */}
          ...
          <aside className="hidden lg:block space-y-4">
              ...
              <div className="bg-card border border-border rounded-lg overflow-hidden">
                ...
                <div className="p-2 space-y-0.5">
                  {categories.map((cat, i) => (
                    <Link key={i} href={`/blogs?category=${cat}`}
                      className="flex items-center justify-between px-3 py-1.5 rounded text-sm text-foreground hover:bg-muted hover:text-primary transition group">
                      <span>{cat}</span>
            ...
            <div className="bg-gradient-to-br from-primary to-indigo-700 rounded-lg p-4 text-white">
              <div className="flex items-center gap-2 mb-1.5">
                ...
                <h3 className="font-bold text-sm uppercase tracking-wide">Newsletter</h3>
              </div>
                ...
                <div className="px-4 py-2.5 border-b border-border bg-muted/30 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-sm uppercase tracking-wide text-foreground">
                      {relatedArticles.length > 0 ? 'Related Articles' : 'More Articles'}
                    </h3>
                  </div>
                  <Link href="/blogs" className="text-xs text-primary hover:underline flex items-center gap-0.5">
                  ...
                  {displayRelated.map(rel => (
                    <Link key={rel.id} href={`/blogs/${rel.slug}`} className="flex gap-3 group hover:bg-muted/40 rounded p-1.5 -mx-1.5 transition">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[11px] text-muted-foreground mt-0.5 flex items-center gap-1">
                          <Calendar className="h-3 w-3" />{formatDateShort(rel.published_at)}
                        </p>
                      </div>
                    </Link>
                  ))}
      ...
      {/* ── Mobile: Floating TOC button + bottom drawer ── */}
      {tocItems.length > 0 && (
        <div className="lg:hidden">
          ...
          <div className={`fixed bottom-0 left-0 right-0 z-50 bg-card border-t border-border rounded-t-2xl shadow-2xl transition-transform duration-300 ${tocOpen ? 'translate-y-0' : 'translate-y-full'}`}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-border">
              ...
              <button onClick={() => setTocOpen(false)} className="p-1 rounded hover:bg-muted transition">
      ...
      {/* Floating TOC button — mobile only, left side */}
          <button
