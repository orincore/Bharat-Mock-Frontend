...
export default function ExamDetailPage() {
  ...
  const examParam = (params?.id as string) ?? (params?.slug as string);
  ...
  const fetchExamDetails = async () => {
    ...
    try {
      if (!examParam) {
        setError('Invalid exam reference');
        setIsLoading(false);
        return;
      }

      const data = await examService.getExamById(examParam);
      setExam(data);
    } catch (err: any) {
  ...
  const handleStartExam = async () => {
    ...
    try {
      const targetExamId = exam?.id || examParam;
      if (!targetExamId) {
