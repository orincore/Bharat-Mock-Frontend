...
interface InitialData {
  ...
  pageContent: any;
  customTabs: any[];
  sidebarSeries: TestSeries[];
  sidebarBanner: PageBanner | null;
}
...
function MobileTOC({
  ...
  return (
    <div className="lg:hidden">
      ...
      <div className={`fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-200 rounded-t-2xl shadow-2xl transition-transform duration-300 ${open ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <div className="flex items-center gap-2">
            <List className="h-4 w-4 text-blue-600" />
            <span className="font-bold text-sm text-slate-900">Table of Contents</span>
          </div>
          <button onClick={() => setOpen(false)} className="p-1 rounded hover:bg-slate-100 transition">
        ...
        <nav className="overflow-y-auto max-h-[60vh] pb-safe p-3 space-y-0.5">
          {sections.map((section: any, idx: number) => (
            <button
              key={`${section.id}-${idx}`}
              type="button"
              onClick={() => { onScrollTo(section.id); setOpen(false); }}
              className={`w-full flex items-start gap-2 px-2 py-1.5 rounded text-xs transition text-left group ${
      ...
      <button
        ...
        className="fixed bottom-5 left-4 z-40 flex items-center gap-2 bg-blue-600 text-white px-4 py-2.5 rounded-full shadow-lg text-sm font-semibold"
      >
...
export default function TestSeriesDetailClient({ initialData, slug }: { initialData: InitialData; slug: string }) {
  ...
  const [sidebarSeries, setSidebarSeries] = useState<TestSeries[]>(initialData.sidebarSeries);
  const [sidebarBanner, setSidebarBanner] = useState<PageBanner | null>(initialData.sidebarBanner);
  const [sidebarLoading, setSidebarLoading] = useState(false);
  const [globalTab, setGlobalTab] = useState<string>('overview');
  const [pageContent, setPageContent] = useState<any>(initialData.pageContent);
  const [customTabs, setCustomTabs] = useState<any[]>(initialData.customTabs);
  ...
  // Scroll spy for TOC
  useEffect(() => {
    const sections = getSectionsForTab(globalTab);
    if (sections.length === 0) return;

    const handleScroll = () => {
      const sectionElements = sections.map((s: any) =>
    ...
    <div className="min-h-screen bg-[#f5f7fb]">
      ...
      <div className="container-main py-6 overflow-hidden">
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_260px] items-start">
          ...
          {getSectionsForTab(globalTab).length > 0 && (
            <MobileTOC
              sections={getSectionsForTab(globalTab)}
            ...
            {getSectionsForTab(globalTab).length > 0 && (
              <div className="hidden lg:block bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wide">Contents</h3>
                </div>
