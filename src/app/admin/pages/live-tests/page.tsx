...
import { Loader2, ImagePlus, Link2, Type, Trash2, Edit } from 'lucide-react';

const PAGE_IDENTIFIER = 'live_tests_hero';

export default function LiveTestsAdminPage() {
  ...
  const fetchBanners = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: error.message || 'Failed to load hero banners', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast({ title: 'Error', description: error.message || 'Failed to upload banner image', variant: 'destructive' });
    } finally {
      setUploading(false);
      event.target.value = '';
    }
  ...
  const handleSave = async () => {
    if (!form.imageUrl.trim()) {
      toast({ title: 'Image required', description: 'Please upload a hero banner image', variant: 'destructive' });
      return;
    }
    ...
    try {
      if (editingBanner) {
        ...
        toast({ title: 'Created', description: 'New hero banner added' });
      }
      resetForm();
    } catch (error: any) {
      toast({ title: 'Error', description: error.message || 'Failed to save banner', variant: 'destructive' });
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleToggleActive = async (banner: PageBanner) => {
    try {
      const updated = await pageBannersService.updateBanner(banner.id, { isActive: !banner.is_active });
  ...
  const handleEdit = (banner: PageBanner) => {
    ...
    setForm({ imageUrl: banner.image_url, linkUrl: banner.link_url || '', altText: banner.alt_text || '' });
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-2">
          <Breadcrumbs
            items={[AdminBreadcrumb(), { label: 'Pages', href: '/admin/pages' }, { label: 'Live Tests Page' }]}
            className="mb-2"
          />
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-slate-400">Live Tests</p>
            <h1 className="font-display text-3xl font-bold text-slate-900">Hero Banner Management</h1>
            <p className="text-muted-foreground">Upload highlight artwork that appears on the live tests hero section.</p>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <Card className="p-6 space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-slate-900">{editingBanner ? 'Edit Banner' : 'Add New Banner'}</h2>
              <p className="text-sm text-muted-foreground">Showcase live windows, featured fixtures, or spotlight partners.</p>
            </div>
            {editingBanner && (
          ...
          <Button onClick={handleSave} disabled={saving} className="w-full sm:w-auto">
            {saving ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : editingBanner ? <Edit className="h-4 w-4 mr-2" /> : <ImagePlus className="h-4 w-4 mr-2" />}
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Activate the hero artwork you want to surface on the live tests page.</p>
            </div>
          </div>
              ...
              {banners.map((banner) => (
                ...
                >
                  ...
                  <img
                    ...
                    alt={banner.alt_text || 'Live tests banner'}
                    className="h-32 w-full md:w-64 object-cover rounded-xl border border-slate-100"
                  />
                  <div className="flex-1 w-full">
                    <p className="text-sm font-semibold text-slate-900">{banner.alt_text || 'Untitled banner'}</p>
                    {banner.link_url && (
                      <a href={banner.link_url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 underline">
                        {banner.link_url}
                      </a>
                    )}
                    <p className="text-xs text-slate-500 mt-1">Updated {new Date(banner.updated_at).toLocaleDateString()}</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2 text-sm text-slate-600">
                      <Switch
                        ...
                        id={`hero-banner-active-${banner.id}`}
                      />
                      <label htmlFor={`hero-banner-active-${banner.id}`}>Active</label>
                    </div>
