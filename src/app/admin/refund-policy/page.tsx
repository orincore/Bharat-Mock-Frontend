...
import { fallbackRefundPolicy } from '../../../lib/constants/refund';
import {
  RefundPolicyContent,
  RefundPolicyData,
  RefundPolicyPoint,
...
interface UIPoint extends Omit<RefundPolicyPoint, 'list_items'> {
  localId: string;
  sectionLocalId: string;
  listItems: UIListItem[];
}

interface UISection extends Omit<RefundPolicySection, 'points'> {
  localId: string;
  points: UIPoint[];
}
...
const toListItems = (items?: RefundPolicyPoint['list_items']): UIListItem[] =>
  (items || []).map((item) =>
    typeof item === 'string'
...
const deserializeSections = (data: RefundPolicyData): UISection[] => {
  const sourceSections = data.sections?.length ? data.sections : fallbackRefundPolicy.sections;
...
const SectionCard = ({
  ...
  index: number;
  total: number;
  onSectionChange: (localId: string, field: keyof RefundPolicySection, value: string) => void;
  ...
  onPointChange: (sectionLocalId: string, pointLocalId: string, field: keyof RefundPolicyPoint, value: string) => void;
...
export default function RefundPolicyAdminPage() {
  ...
  const [content, setContent] = useState<RefundPolicyContent>(fallbackRefundPolicy.content!);
  ...
  const hydrate = (data: RefundPolicyData) => {
    setContent(data.content ?? fallbackRefundPolicy.content!);
  ...
  const loadPolicy = async () => {
    try {
      setLoading(true);
      const data = await adminService.getRefundPolicyAdmin();
      hydrate(data);
    } catch (error: any) {
      console.error(error);
      toast({
        title: 'Unable to load Refund Policy',
        description: error?.message || 'Loaded fallback data instead.',
        variant: 'destructive'
      });
      hydrate(fallbackRefundPolicy);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const saved = await adminService.upsertRefundPolicy(payload);
      hydrate(saved);
      toast({ title: 'Refund Policy updated successfully' });
    } catch (error: any) {
      console.error(error);
      toast({
        title: 'Save failed',
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-wide text-primary font-semibold mb-1">Refund Policy</p>
          <h1 className="font-display text-3xl font-bold">Refund Policy Management</h1>
          <p className="text-muted-foreground max-w-2xl">
            Edit the public-facing Refund Policy content, sections, and bullet points. Changes go live as soon as you save.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          <Button type="submit" form="refund-form" disabled={saving}>
            ...
            <Card className="p-6 space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Set the headline content that appears at the top of the public Refund Policy page.
                </p>
              </div>
          ...
          <TabsContent value="sections" className="space-y-6 mt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Manage each section of the Refund Policy and its supporting bullet points.
                </p>
              </div>
