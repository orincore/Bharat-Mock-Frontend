...
import { GripVertical, Loader2, Plus, RefreshCcw, Save, Trash2 } from 'lucide-react';

import { adminService } from '@/lib/api/adminService';
import { fallbackDisclaimer } from '@/lib/constants/disclaimer';
import { DisclaimerContent, DisclaimerData, DisclaimerPoint, DisclaimerSection } from '@/types';
...
interface UIPoint extends Omit<DisclaimerPoint, 'list_items'> {
  localId: string;
  sectionLocalId: string;
  listItems: UIListItem[];
}

interface UISection extends Omit<DisclaimerSection, 'points'> {
  localId: string;
  points: UIPoint[];
}
...
const toListItems = (items?: DisclaimerPoint['list_items']): UIListItem[] =>
  (items || []).map((item) =>
    typeof item === 'string'
...
const deserializeSections = (data: DisclaimerData): UISection[] => {
...
const SectionCard = ({
  ...
  index: number;
  total: number;
  onSectionChange: (localId: string, field: keyof DisclaimerSection, value: string) => void;
  ...
  onPointChange: (sectionLocalId: string, pointLocalId: string, field: keyof DisclaimerPoint, value: string) => void;
  ...
  <Card className="p-5 space-y-4">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <div className="space-y-2">
        <Label>Title</Label>
        <Input value={section.title} onChange={(e) => onSectionChange(section.localId, 'title', e.target.value)} />
      </div>
      <div className="space-y-2">
...
export default function DisclaimerAdminPage() {
  ...
  const [content, setContent] = useState<DisclaimerContent>(fallbackDisclaimer.content!);
  ...
  const hydrate = (data: DisclaimerData) => {
    setContent(data.content ?? fallbackDisclaimer.content!);
  ...
  const loadDisclaimer = async () => {
    try {
      setLoading(true);
      const data = await adminService.getDisclaimerAdmin();
      hydrate(data);
    } catch (error: any) {
      console.error(error);
      toast({
  ...
  const updateSection = (localId: string, updater: (section: UISection) => UISection) => {
    setSections((prev) => prev.map((section) => (section.localId === localId ? updater(section) : section)));
  };

  const updatePoint = (sectionLocalId: string, pointLocalId: string, updater: (point: UIPoint) => UIPoint) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!content.title?.trim()) {
      toast({
        ...
        description: 'Provide a disclaimer title before saving.',
        variant: 'destructive'
      });
      return;
    }
    ...
    if (!content.last_updated) {
      toast({
        ...
        description: 'Select the last updated date for the disclaimer.',
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      const saved = await adminService.upsertDisclaimer(payload);
      hydrate(saved);
      toast({ title: 'Disclaimer updated successfully' });
    } catch (error: any) {
      console.error(error);
      toast({
        title: 'Save failed',
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-wide text-primary font-semibold mb-1">Disclaimer</p>
          <h1 className="font-display text-3xl font-bold">Disclaimer Management</h1>
          <p className="text-muted-foreground max-w-2xl">
            Edit the public-facing Disclaimer sections, bullet points, and intro content. Changes go live as soon as you save.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          <Button type="button" variant="outline" onClick={loadDisclaimer} disabled={loading || saving}>
            <RefreshCcw className="h-4 w-4 mr-2" /> Refresh
          </Button>
          <Button type="submit" form="disclaimer-form" disabled={saving}>
      ...
      <form id="disclaimer-form" onSubmit={handleSubmit} className="space-y-6">
        <Tabs defaultValue="content">
          ...
          <TabsContent value="content" className="space-y-6 mt-6">
            <Card className="p-6 space-y-4">
              <div>
                <h2 className="font-display text-xl font-bold">Disclaimer Overview</h2>
                <p className="text-sm text-muted-foreground">
                  Set the headline content that appears at the top of the public Disclaimer page.
                </p>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                  <Label htmlFor="disclaimer-date">Last Updated</Label>
                  <Input
                    id="disclaimer-date"
                    type="date"
              ...
              <div className="space-y-2">
                <Label htmlFor="disclaimer-intro">Intro Paragraph</Label>
                <Textarea
                  id="disclaimer-intro"
                  rows={5}
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="disclaimer-email">Contact Email</Label>
                  <Input
                    id="disclaimer-email"
                    type="email"
                ...
                <div className="space-y-2">
                  <Label htmlFor="disclaimer-url">Contact URL</Label>
                  <Input
                    id="disclaimer-url"
          ...
          <TabsContent value="sections" className="space-y-6 mt-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Manage each section of the Disclaimer and its supporting bullet points.
                </p>
              </div>
